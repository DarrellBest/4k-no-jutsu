...
Everything's Vulkan-based, not CUDA, so there's no GPU driver/toolkit
version matching required. Works even without a GPU via software rendering
(e.g. mesa lavapipe), just much slower; see [Performance](#performance).

Prefer a UI over the CLI? See [Desktop app](#desktop-app) below: it can run
this same setup for you on first launch.
...
| Flag | What it does |
|---|---|
| `--target-resolution` | Letterbox/pad the upscaled output to an exact size: `4k`/`uhd` (3840×2160), `1080p`, `720p`, or `WIDTHxHEIGHT` (e.g. `3200x1800`). AI backends only support fixed integer scale factors (2×/3×/4×), so this is what actually lands you on a standard size without distortion. Omit it to keep the backend's native scaled-but-unpadded output. |
| `--max-workers N` | Process N windows concurrently. The AI backend binaries are usually far from saturating a real GPU one window at a time; benchmark on your own hardware, then pick a number. Going too high just shifts the bottleneck to CPU/disk instead. |
...
so there's no flicker or drift between frames. Windows process
independently (disjoint frame/segment paths), which is what makes
`--max-workers` concurrency and per-window resumability both safe.
...
Both AI backends are Vulkan-based ncnn binaries, not CUDA, so there's no GPU
driver version matching against a specific toolkit release.
...
processing one window at a time (observed as low as 3 to 20% GPU
utilization at `--max-workers 1`), so concurrency gives a large real
speedup, until CPU/disk becomes the new bottleneck instead. Benchmark a
short clip on your own hardware (`jutsu run` on a `--duration`-trimmed
source, or `jutsu compare`) before picking a worker count for a
full-length run.
...
- **Run**: pick a source file, profile, target resolution, and worker
  count; watch live progress and logs; secure mode fields appear when
  selected.
- **Compare**: run the same model comparison as `jutsu compare`, then open
  the HTML report straight from the app.
- **Setup**: live status for the conda env / `jutsu` install / AI
  backends, with a one-click "Run setup" that does the same thing as
  `scripts/setup.sh`, so a first-time user never has to touch a terminal.
- **Settings**: pCloud remote, Jellyfin folder, and default secure-mode
  ramfs cap, persisted locally and injected into every job the app runs.
